...
| 2026-06-26 | [UI] | Contact form legibility: `bg-dark-bg/60` compiled to invalid CSS (var-colour + opacity) and fell back to a **white** input, hiding the near-white text. Switched to `bg-white/5` (channel-based → renders dark) and `placeholder-dark-muted`, so typed text/placeholder are clearly visible; refreshed the placeholders with friendlier, Python-flavoured copy. |
